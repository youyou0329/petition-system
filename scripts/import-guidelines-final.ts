...
async function main() {
  ...
  for (const file of FILES) {
    ...
    try {
      ...
      // 使用 coze-coding-ai 命令插入数据库
      const escapedContent = escapeSql(textContent);
      const sql = `INSERT INTO guideline_docs (title, category, content, is_active, created_at) VALUES ('${escapeSql(file.name)}', '${escapeSql(file.category)}', '${escapedContent}', true, NOW());`;
      
      // 保存 SQL 到临时文件
      const fs = await import('fs');
      const sqlFile = `/tmp/guideline_${Date.now()}.sql`;
      fs.writeFileSync(sqlFile, sql);
      
      // 执行 SQL
      const result = execSync(`coze-coding-ai db execute --file ${sqlFile}`, { encoding: 'utf-8' });
      console.log(`  ✅ 保存成功`);
      successCount++;
